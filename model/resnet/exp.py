...
class SFConv2d(nn.Conv2d):
    """Spatial-Frequency 2D Convolutions"""
    def __init__(self, 
                 in_channels, 
                 out_channels,
                 kernel_size,
        ...
        self.freq_conv = nn.Conv2d(in_channels*2, out_channels*2, kernel_size=1, bias=False)
        self.sf_coef = nn.Parameter(torch.tensor(-10., requires_grad=True))

    def forward(self, x):
        size = x.shape[-2:]
        # spatial branch
        spat_x = F.conv2d(x, self.weight, self.bias, 
...
class Conv2d_cd(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, theta=0.7, **kwargs):
...
class BasicBlock(nn.Module):
    ...
    def __init__(
            ...
            attn_layer=None, aa_layer=None, drop_block=None, drop_path=None, freq_norm=None):
        ...
        if freq_norm is not None and inplanes == first_planes:
            conv_func = partial(SFConv2d, freq_norm=freq_norm)
        else:
            conv_func = nn.Conv2d
        ...
        if freq_norm is not None and first_planes == outplanes:
            conv_func = partial(SFConv2d, freq_norm=freq_norm)
        else:
            conv_func = nn.Conv2d
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if freq_norm is not None and inplanes == first_planes:
            ...
            conv_func = nn.Conv2d
        self.conv1 = conv_func(inplanes, first_planes, kernel_size=1, bias=False)
        self.bn1 = norm_layer(first_planes)
        self.act1 = act_layer(inplace=True)

        if freq_norm is not None and first_planes == width:
            conv_func = partial(SFConv2d, freq_norm=freq_norm)
        else:
            conv_func = nn.Conv2d
        self.conv2 = conv_func(
        ...
        if freq_norm is not None and width == outplanes:
            conv_func = partial(SFConv2d, freq_norm=freq_norm)
        else:
            conv_func = nn.Conv2d
        self.conv3 = conv_func(width, outplanes, kernel_size=1, bias=False)
...
def make_blocks(
    ...
    freq_norm = kwargs.pop("freq_norm", None)
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        block_kwargs = dict(
            ...
            dilation=dilation, 
            drop_block=db, 
            freq_norm=freq_norm if stage_idx > 0 else None,
            **kwargs
        )
        blocks = []
...
def check_for_sfconv_params(missing_keys):
    temp_flag = True
    for i in missing_keys:
        if "sf_coef" in i or "freq_conv" in i:
            pass
        else:
            temp_flag = False
            break
...
def custom_resnet18(weights_path=None, freq_norm=None):
    model_args = dict(block=BasicBlock, layers=[2, 2, 2, 2], block_args=dict(freq_norm=freq_norm))
    net = _create_resnet('resnet18', pretrained=False, **model_args)
    if weights_path is not None:
        resnet_weights = torch.load(weights_path, map_location="cpu")
        ret = net.load_state_dict(resnet_weights, strict=False)
        if not ret.missing_keys:
            pass  # good condition
        elif not check_for_sfconv_params(ret.missing_keys):
            raise RuntimeError('Missing keys when loading pretrained weights: {}'.format(ret.missing_keys))
        assert not ret.unexpected_keys, 'Missing keys when loading pretrained weights: {}'.format(ret.unexpected_keys)
        print(f'Loaded pretrained weights for ResNet-18 from {weights_path}.')
    return net


def custom_resnet50(weights_path=None, freq_norm=None):
    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], block_args=dict(freq_norm=freq_norm))
    net = _create_resnet('resnet50', pretrained=False, **model_args)
    if weights_path is not None:
        resnet_weights = torch.load(weights_path, map_location="cpu")
        ret = net.load_state_dict(resnet_weights, strict=False)
        if not ret.missing_keys:
            ...
            raise RuntimeError('Missing keys when loading pretrained weights: {}'.format(ret.missing_keys))
        assert not ret.unexpected_keys, 'Missing keys when loading pretrained weights: {}'.format(ret.unexpected_keys)
        print(f'Loaded pretrained weights for ResNet-50 from {weights_path}.')
    return net
